...
export function App() {
 const store=useExperienceStore(); const setProgress=useExperienceStore(state=>state.setProgress); const setChapter=useExperienceStore(state=>state.setChapter); const [entered,setEntered]=useState(false); const [menu,setMenu]=useState(false);
 useEffect(()=>{document.body.style.overflow=entered?'':'hidden'; return()=>{document.body.style.overflow=''}},[entered]);
 useEffect(()=>{const update=()=>{const max=document.documentElement.scrollHeight-innerHeight;const p=max?scrollY/max:0;const chapter=chapterForProgress(p);setProgress(p);setChapter(chapter)};addEventListener('scroll',update,{passive:true});update();return()=>removeEventListener('scroll',update)},[setProgress,setChapter]);
 const chapter=chapters[store.currentChapter]; const jump=(i:number)=>{scrollTo({top:(document.documentElement.scrollHeight-innerHeight)*CHAPTER_CUES[i],behavior:store.reducedMotion?'auto':'smooth'});setMenu(false)};
 ...
 <header className="topbar"><div className="brand">BEYOND <span>—</span> <small>INTERSTELLAR STUDIES</small></div><div className="controls"><IconButton label="Toggle sound" onClick={store.toggleSound} active={store.soundEnabled}>{store.soundEnabled?'◖))':'◖×'}</IconButton><IconButton label="Pause animation" onClick={store.togglePause}>{store.isPaused?'▶':'Ⅱ'}</IconButton><IconButton label="Toggle reduced motion" onClick={store.toggleMotion} active={store.reducedMotion}>◌</IconButton><IconButton label="Open chapters" onClick={()=>setMenu(!menu)}>☰</IconButton></div></header>
